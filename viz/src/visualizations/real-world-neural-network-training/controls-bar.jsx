      ...
      <div className='flex flex-wrap gap-3 md:gap-4 items-end'>
        ...
        <div className='flex-shrink-0 w-full sm:w-auto flex gap-2'>
          <button
            ...
            disabled={isTraining}
            className='flex-1 sm:flex-none sm:w-20 bg-slate-600 text-slate-200 hover:bg-slate-500 disabled:opacity-50 font-bold py-2 px-3 rounded-lg shadow-sm transition-colors text-xs md:text-sm'
          >
            Reset
          </button>
          {isTraining ? (
        ...
        <div className='flex flex-col w-full sm:w-auto'>
          <div className='flex items-center justify-between mb-1 gap-3'>
            <label className='text-[10px] font-bold text-slate-400 uppercase'>Target Dataset</label>
          ...
          <select
            ...
            className='bg-slate-700 border border-slate-600 text-slate-200 rounded-md px-2 py-1.5 text-xs md:text-sm font-semibold'
          >
        ...
        {/* Hidden Layers */}
        <div className='flex flex-col w-full sm:w-auto'>
          <label className='text-[10px] font-bold text-slate-400 uppercase mb-1'>Hidden Layers</label>
          <select
            value={depth}
        ...
        <div className='flex gap-2 w-full sm:w-auto'>
          {[...Array(depth)].map((_, i) => (
            <div key={i} className='flex flex-col'>
              <label className='text-[10px] font-bold text-violet-400 uppercase mb-1'>L{i + 1} Neurons</label>
              <select
                value={neurons[i]}
                onChange={e => {
                  const n = [...neurons];
                ...
                disabled={isTraining}
                className='bg-violet-900/20 border border-violet-600 text-violet-200 rounded-md px-2 py-1.5 text-xs font-mono shadow-inner'
              >
        ...
        {/* Max Epochs */}
        <div className='flex flex-col w-full sm:w-auto'>
          <label className='text-[10px] font-bold text-slate-400 uppercase mb-1'>Max Epochs</label>
          <select
            value={maxEpochs}
        ...
        {/* Epoch + Loss Readout */}
        <div className='flex items-center justify-center gap-3 bg-slate-700 px-3 py-1.5 md:px-4 md:py-2 rounded-lg border border-slate-600 w-full lg:w-auto ml-auto'>
          <div className='text-center'>
            <span className='text-slate-400 font-semibold uppercase tracking-wider text-[9px] md:text-[10px]'>Epoch</span>
            <div className='font-mono text-base md:text-lg font-bold text-slate-200 leading-none mt-0.5'>{epoch}</div>
          </div>
          <div className='w-px h-6 md:h-8 bg-slate-500' />
          <div className='text-center w-16 md:w-20'>
            <span className='text-slate-400 font-semibold uppercase tracking-wider text-[9px] md:text-[10px]'>Mean Loss</span>
