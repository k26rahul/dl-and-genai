...
const functions = [
  {
    ...
    fStr: '(x₁-1)² + (x₂+2)² + x₃² + (x₄-2)² + (x₅+1)²',
    gradStr: '[2(x₁-1), 2(x₂+2), 2x₃, 2(x₄-2), 2(x₅+1)]',
    domain: [-5, 5],
  },
  {
    id: 'elliptic',
    ...
    fStr: '2x₁² + 0.5(x₂-1)² + 3(x₃+1)² + (x₄-3)² + 1.5x₅² + 5',
    gradStr: '[4x₁, 1(x₂-1), 6(x₃+1), 2(x₄-3), 3x₅]',
    domain: [-5, 5],
  },
];
  ...
  return (
    <div className='min-h-screen bg-slate-50 text-slate-800 p-1 sm:p-2 md:p-4 font-sans'>
      <div className='max-w-7xl mx-auto'>
        ...
        {/* Interactive Layout (Mobile reordered single col, Desktop two col) */}
        <div className='flex flex-col lg:grid lg:grid-cols-12 gap-2 md:gap-4 lg:gap-5'>
          {/* LEFT COLUMN: LOSS CURVE & SELECTOR */}
          <div className='contents lg:flex lg:flex-col lg:col-span-6 lg:gap-5'>
            {/* The Loss Curve Plot (ORDER 1) */}
            <div className='order-1 lg:order-none bg-white p-2 md:p-4 rounded-xl md:rounded-2xl shadow-sm border border-slate-200'>
              <h2 className='text-xs md:text-sm font-bold text-slate-700 uppercase tracking-wide mb-2 md:mb-4 px-1 md:px-2'>
                Training Progress (Loss Curve)
              </h2>
              <div className='w-full flex justify-center bg-slate-50 rounded-lg md:rounded-xl border border-slate-100 overflow-hidden'>
                <svg
              ...
              <div className='mt-2 md:mt-4 flex justify-between px-1 md:px-2'>
            ...
            {/* Function Selector Card (ORDER 3) */}
            <div className='order-3 lg:order-none bg-white p-2 md:p-4 rounded-xl md:rounded-2xl shadow-sm border border-slate-200'>
              ...
              <div className='mb-2 md:mb-4 bg-slate-50 p-2 md:p-3 rounded-lg md:rounded-xl border border-slate-200 flex flex-col gap-1 md:gap-1.5 font-mono text-[10px] md:text-[13px] shadow-inner overflow-x-auto whitespace-nowrap'>
                <div className='flex items-start gap-1 md:gap-2'>
                  <span className='font-bold text-blue-600 w-10 md:w-16 shrink-0'>
                    ƒ(X)
                  </span>
                  <span className='text-slate-700'>= {currentFunc.fStr}</span>
                </div>
                <div className='flex items-start gap-1 md:gap-2 mt-1 border-t border-slate-200 pt-1'>
                  <span className='font-bold text-purple-600 w-10 md:w-16 shrink-0'>
                    ∇ƒ(X)
                  </span>
                  <span className='text-slate-700'>= {currentFunc.gradStr}</span>
                </div>
              </div>
          ...
          <div className='contents lg:flex lg:flex-col lg:col-span-6 lg:gap-5'>
            {/* 5 Parameters Readout & Controls (ORDER 2) */}
            <div className='order-2 lg:order-none bg-white p-2 md:p-4 rounded-xl md:rounded-2xl shadow-sm border border-slate-200'>
              <div className='border-b pb-1.5 md:pb-3 mb-2 md:mb-4'>
              ...
              <div className='space-y-1.5 md:space-y-3'>
                {params.map((val, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className='flex flex-col gap-0.5 md:gap-1 bg-slate-50 p-1.5 md:p-2.5 rounded-lg border border-slate-100'
                    >
                      <div className='flex justify-between items-center'>
                        ...
                        <input
                          ...
                          className='flex-1 mx-2 md:mx-4 h-1 md:h-1.5 bg-slate-300 rounded-lg appearance-none cursor-pointer accent-blue-600'
                        />
                        <span className='font-mono text-[11px] md:text-sm font-semibold w-8 md:w-12 text-right text-blue-900'>
                          {val.toFixed(2)}
                        </span>
                      </div>
            ...
            {/* Gradient Descent Controls (ORDER 4) */}
          ...
          <div className='bg-blue-50 p-3 md:p-6 rounded-xl md:rounded-2xl border border-blue-100 flex flex-col gap-2 md:gap-4'>
            ...
            <div className='text-xs md:text-sm text-blue-900 space-y-3 md:space-y-4 max-w-5xl mx-auto w-full'>
              <div className='space-y-1 md:space-y-2'>
                ...
                surfaces. Here, we have <strong>5 inputs and 1 output</strong>. This forms
                a mathematical surface in <strong>6-Dimensional space</strong>! Since
                human brains cannot visualize 6D space, we cannot plot the "surface"
                anymore.
              </div>

              <div className='space-y-1 md:space-y-2'>
                <strong>2. The Epoch vs. Output Plot:</strong> Because we can't see the 6D
                landscape, we track our progress using a <strong>Loss Curve</strong>.
                ...
                walking "downhill" in 6D space, even if we can't physically see the hill!
              </div>
              ...
              <div className='bg-white p-2.5 md:p-4 rounded-lg border border-blue-100 shadow-sm space-y-1.5 md:space-y-2'>
                ...
                <ul className='list-disc pl-4 md:pl-5 space-y-1 md:space-y-2 text-slate-700'>
                  <li>
                    We calculate the <strong>partial derivative (∇ƒ)</strong> for every
                    single parameter.
                  </li>
                  <li>
                    <strong>
                  ...
                  <li>
                    ...
                    <code className='bg-slate-100 px-1 rounded text-indigo-600 font-bold text-[10px] md:text-sm leading-loose'>
                      X<sub>new</sub> = X<sub>curr</sub> - (LR × ∂Z/∂X)
                    </code>
                  </li>
                </ul>
              </div>
              ...
              <div className='space-y-1 md:space-y-2'>
                ...
                moved away from the minimum). Then, hit <strong>Auto</strong> and watch
                the algorithm automatically slide all 5 parameters back to their perfect,
                optimal positions!
              </div>
            </div>
          </div>
