...
export default function ThreeCanvas({ mountRef, threeLoaded, currentFunc, handleManualChange, x, y }) {
  return (
    <div className='order-1 lg:order-none bg-slate-800 p-2 md:p-3 rounded-lg md:rounded-xl shadow-sm border border-slate-700'>
      <div
        ref={mountRef}
        className='w-full h-64 sm:h-80 md:h-[400px] bg-slate-900 rounded-lg overflow-hidden cursor-move relative'
      >
        {!threeLoaded && (
      ...
      <div className='text-center text-[10px] md:text-xs text-slate-500 mt-1.5'>
      ...
      <div className='mt-2 md:mt-3 px-1 md:px-2 space-y-2 md:space-y-3'>
        <div>
          <label className='flex justify-between text-[11px] md:text-xs font-semibold text-slate-300 mb-1'>
            ...
            <span className='text-rose-400 font-mono'>{x.toFixed(4)}</span>
          </label>
          <input
            type='range'
            ...
            className='w-full h-1.5 md:h-2 bg-slate-600 rounded-lg appearance-none cursor-pointer accent-rose-500'
          />
        </div>
        <div>
          <label className='flex justify-between text-[11px] md:text-xs font-semibold text-slate-300 mb-1'>
            <span>Parameter Y</span>
            <span className='text-sky-400 font-mono'>{y.toFixed(4)}</span>
          </label>
          <input
            type='range'
            min={currentFunc.domain[0]}
            ...
            className='w-full h-1.5 md:h-2 bg-slate-600 rounded-lg appearance-none cursor-pointer accent-sky-500'
          />
        </div>
      </div>
    </div>
