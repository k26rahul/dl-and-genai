...
export default function FunctionSelector({
  ...
  return (
    <div className='order-3 lg:order-none bg-slate-800 p-2 md:p-3 rounded-lg md:rounded-xl shadow-sm border border-slate-700'>
      {/* Formula Readout */}
      <div className='mb-2 md:mb-3 bg-slate-900 border border-slate-600 rounded-lg p-2 md:p-3 flex flex-col gap-1 md:gap-1.5 shadow-inner overflow-x-auto'>
        <div className='flex items-start gap-1.5 md:gap-2 min-w-0'>
          ...
          <span className='text-slate-300 text-[10px] md:text-xs break-all'>
            <TeX math={currentFunc.fLatex} />
          </span>
        </div>
        <div className='flex items-start gap-1.5 md:gap-2 min-w-0 border-t border-slate-600 pt-1'>
          <span className='font-bold text-violet-400 font-mono text-xs md:text-sm shrink-0'>
            <TeX math='\nabla f(\mathbf{X})' />
          </span>
          <span className='text-slate-500 shrink-0 text-xs mt-0.5'>=</span>
      ...
      <select
        className='w-full bg-slate-700 border border-slate-600 text-slate-200 text-xs md:text-sm rounded-lg p-1.5 md:p-2 shadow-sm focus:ring-violet-500 focus:border-violet-500 cursor-pointer'
